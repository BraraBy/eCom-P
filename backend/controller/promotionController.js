...
const err = (res, message = "Server Error", status = 500) =>
  res.status(status).json({ status: String(status), message });
...
const createPromotion = async (data) => {
  ...
  if (kind === "percent" && !(Number(discount_percent) > 0 && Number(discount_percent) <= 100)) {
    const e = new Error("discount_percent must be 0 < x <= 100");
    e.statusCode = 400; throw e;
  }
  if (kind === "amount" && !(Number(discount_amount) >= 0)) {
    const e = new Error("discount_amount must be >= 0");
    e.statusCode = 400; throw e;
  }
  ...
  try {
    ...
    `;
    const params = [
      title, description ?? null, code, kind,
...
const updatePromotion = async (promotion_id, data) => {
  const {
    ...
    discount_percent, discount_amount,
    min_order_amount, starts_at, ends_at,
    active, max_total_uses, max_uses_per_user,
  } = data || {};
  ...
  try {
    ...
    `;
    const params = [
      promotion_id, title ?? null, description ?? null,
      code ?? null, kind ?? null,
...
const setPromotionScope = async (promotion_id, data) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM promotion_products WHERE promotion_id=$1`, [promotion_id]);
    await client.query(`DELETE FROM promotion_categories WHERE promotion_id=$1`, [promotion_id]);
...
const redeemPromotion = async (promotion_id, data) => {
  ...
  try {
    ...
    if (customers_id && active.rows[0].max_uses_per_user != null) {
      const used = await client.query(
        `SELECT COUNT(*)::int AS used_by_user
         FROM promotion_redemptions WHERE promotion_id=$1 AND customers_id=$2`,
        [promotion_id, customers_id]
      );
      if ((used.rows[0]?.used_by_user ?? 0) >= active.rows[0].max_uses_per_user) {
        await client.query("ROLLBACK");
        const e = new Error("User quota exceeded");
        e.statusCode = 409; throw e;
      }
    }

    await client.query(
      `INSERT INTO promotion_redemptions (promotion_id,customers_id,order_id) VALUES ($1,$2,$3)`,
...
const validatePromotionByCode = async ({ code, items = [], customers_id = null }) => {
  ...
  try {
    ...
    if (promo.max_total_uses != null) {
      ...
      if ((r.rows[0]?.used ?? 0) >= promo.max_total_uses) {
        const e = new Error("Total quota exceeded");
        e.statusCode = 409; throw e;
      }
    }
    // quota ต่อ user
    if (customers_id && promo.max_uses_per_user != null) {
      ...
      if ((r.rows[0]?.used ?? 0) >= promo.max_uses_per_user) {
        const e = new Error("User quota exceeded");
        e.statusCode = 409; throw e;
      }
    }
    ...
    // scope
    const rPP = await client.query(`SELECT product_id FROM promotion_products WHERE promotion_id=$1`, [promo.promotion_id]);
    const rPC = await client.query(`SELECT category_id FROM promotion_categories WHERE promotion_id=$1`, [promo.promotion_id]);
    ...
    if (minOrder>0 && eligibleSubtotal<minOrder) {
      const e = new Error(`Minimum order ฿${minOrder}`);
      e.statusCode = 409; throw e;
    }
    ...
    await client.query("COMMIT");
    return { promotion: promo, eligibleSubtotal, discount };
  } catch (err) {
    await client.query("ROLLBACK");
    throw err;
  } finally {
...
export default {
  getPromotions, getPromotionById,
  getActivePromotions, countActivePromotions, getActivePromotionByCode,
  createPromotion, updatePromotion, deletePromotion,
  setPromotionScope, redeemPromotion,
